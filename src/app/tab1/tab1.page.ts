...
export class Tab1Page implements OnInit{
  ...
  addFriend(data,i, event: any){
    this.service.showFriends(this.uid).subscribe((doc) =>{
      ...
      else if(!doc.data())
      {
        ...
        this.presentToast('Sukses!','Teman berhasil ditambahkan','primary');
      }
    });
  }
  ...
  addToDb(data,event: any){
    ...
    if(check === 0){
      ...
      this.presentToast('Peringatan!','Teman sudah ditambahkan','danger');
    }
  }
  ...
  removeFriend(data,i,event){
    ...
    this.presentToast('Sukses!','Pertemanan berhasil dihapus','primary');
  }
